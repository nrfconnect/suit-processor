 ...
 */
int suit_validate_common_sequence(cbor_string_type_t *cmd_seq_str);

/** Validate that all commands are supported, and that arguments are sensible.
 */
int suit_validate_command_sequence(cbor_string_type_t *cmd_seq_str);

/** Run all the commands in this sequence.
 */
int suit_run_command_sequence(cbor_string_type_t *cmd_seq_str);
